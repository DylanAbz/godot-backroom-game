...
- **`scripts/monster.gd`** (`Monster`) — loads any GLB, normalizes it to a target height (source models range from 5 cm to 28 m), builds capsule + NavigationAgent3D at runtime, plays the first animation if one exists. Falls back to straight-line pursuit while the navmesh is still baking. Per-type personalities via `cfg.behavior`: `rusher` (hit-and-run), `stalker` (orbits at ~11 m, charges when unwatched or player is weak), `brute` (slow, hears sprinting from afar, knockback-resistant), `smiler` (freezes while watched or flashlit, fast otherwise), `ambusher` (dormant, stares, bursts at close range). Perception helpers: `_sees_player` (LOS ray), `_player_watching` (view cone), `_in_flashlight` (beam cone). hp/damage/atk_cd/stagger_mult come from `MonsterSpawner.TYPES`.
